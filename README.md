...
When `enableValidation` is enabled, generated mocks are automatically validated. If invalid mocks are detected, the system retries generation with AI self-correction (up to 1 retry attempt by default, configurable via the `BEDROCK_GENERATION_MAX_RETRIES` environment variable) and only returns mocks that pass validation.
...
The default API Gateway REST API has a synchronous integration timeout of approximately 29 seconds. This constrains how many AI correction retries can complete within a single request. By default, 1 retry is attempted; this can be overridden at runtime via the `BEDROCK_GENERATION_MAX_RETRIES` environment variable (0-2).
...
| **Max Retries** | N/A | `BEDROCK_GENERATION_MAX_RETRIES` | 0-2 | `1` | Runtime override only. Each retry requires a full Bedrock round-trip |
...
| Configuration | SAM Parameter | Environment Variable | Possible Values | Default | Notes |
|---------------|---------------|---------------------|-----------------|---------|-------|
| **Webhook Timeout** | `WebhookTimeoutSeconds` | N/A | 5, 10, 25, 55, 115 s | `25` | Also drives RuntimeAsync Lambda timeout (value + 5s) and SQS queue visibility (RuntimeAsync timeout × 2) |
