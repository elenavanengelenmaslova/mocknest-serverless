...
> **Stack name length**: Lambda function names are limited to 64 characters. MockNest appends suffixes like `-runtime-async` (14 chars) to the stack name. When deploying via SAR, the `serverlessrepo-` prefix (16 chars) is added automatically. Keep your stack name short to stay within the limit.
