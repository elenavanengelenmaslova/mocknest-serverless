...
- **Application name** (default: `serverlessrepo-MockNest-Serverless-*`): the stack name created via AWS CloudFormation. Auto-generated by SAR.
...
- **RuntimeLambdaTimeout** (default: `10`): timeout in seconds (range: 3–29). Bounded by the API Gateway synchronous integration limit (~29s).

**Generation Lambda** — AI mock generation via Bedrock

- **GenerationLambdaMemorySize** (default: `512`): memory size in MB (range: 256–10240). Default optimized via Lambda Power Tuner. See [PERFORMANCE.md](https://github.com/elenavanengelenmaslova/mocknest-serverless/blob/main/docs/PERFORMANCE.md).
- **GenerationLambdaTimeout** (default: `29`): timeout in seconds (range: 10–900). Default matches the API Gateway synchronous limit. Each retry counts against this timeout.
- **BedrockGenerationMaxRetries** (default: `1`): max retry attempts for AI generation if validation fails (range: 0–2). Each retry requires a full Bedrock round-trip.
- **BedrockModelName** (default: `AmazonNovaPro`): Bedrock model name for AI-powered mock generation. Amazon Nova Pro is officially supported and tested. Other models are experimental. See [Bedrock models](https://docs.aws.amazon.com/bedrock/latest/userguide/models-supported.html).
- **BedrockInferenceMode** (default: `AUTO`): cross-region inference routing mode. `AUTO`, `GLOBAL_ONLY`, `GEO_ONLY`. Use `GEO_ONLY` for strict data residency.

**Webhook / RuntimeAsync Lambda** — asynchronous webhook dispatch

- **RuntimeAsyncLambdaMemorySize** (default: `256`): memory size in MB for the RuntimeAsync Lambda (range: 256–10240). Default optimized via Lambda Power Tuner. See [PERFORMANCE.md](https://github.com/elenavanengelenmaslova/mocknest-serverless/blob/main/docs/PERFORMANCE.md).
- **WebhookTimeoutSeconds** (default: `25`): timeout in seconds for outbound webhook HTTP calls. Allowed: 5, 10, 25, 55, 115. Also drives the RuntimeAsync Lambda timeout (value + 5s) and SQS queue visibility timeout (RuntimeAsync timeout × 6, per AWS best practice).
- **SensitiveHeaders** (default: `x-api-key,authorization,proxy-authorization,x-amz-security-token`): HTTP header names redacted in the S3 request journal. Case-insensitive. Applied to both Runtime and RuntimeAsync Lambda functions.
