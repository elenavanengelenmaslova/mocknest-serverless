...
|-----------|-------------|---------|-------|
| **DeploymentName** | Unique identifier for your deployment | `mocks` | Used as API Gateway stage name (e.g., `my-team-mocks`) |
| **LambdaMemorySize** | Memory allocation in MB | `1024` | Range: 512-10240 MB |
| **LambdaTimeout** | Function timeout in seconds | `120` | Range: 3-900 seconds |
| **BedrockModelName** | AI model for mock generation | `AmazonNovaPro` | Amazon Nova Pro officially supported and tested |
| **BedrockInferenceMode** | Inference profile selection | `AUTO` | AUTO (recommended), GLOBAL_ONLY, or GEO_ONLY |
| **BedrockGenerationMaxRetries** | Max retry attempts for AI generation | `1` | Range: 0-5 retries for validation failures |
...
- **GLOBAL_ONLY**: Forces use of cross-region inference profile only
  ...
  - Uses shared cross-region capacity

- **GEO_ONLY**: Forces use of region-specific inference profile only
  - Best for users with strict data residency requirements
