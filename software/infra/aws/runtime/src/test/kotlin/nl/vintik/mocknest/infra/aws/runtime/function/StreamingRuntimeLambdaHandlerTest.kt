...
class StreamingRuntimeLambdaHandlerTest : KoinTest {
    ...
    @Nested
    inner class S3StreamingBoundedBuffer {

        @Test
        fun `Given S3 buffer size constant When checking Then should be 1MB`() {
            // Given/When/Then — verify the S3 streaming buffer constant is correctly defined
            val expectedBufferSize = 1024 * 1024 // 1MB
            assertEquals(expectedBufferSize, S3ResponseStreamer.BUFFER_SIZE, "S3 stream buffer should be 1MB")
        }

        @Test
        fun `Given client request with bodyFileName When handling Then should stream from S3`() {
            // Given — WireMock response references a bodyFileName stored in S3
            val input = createApiGatewayRequest("/mocknest/api/download", "GET")
            val output = ByteArrayOutputStream()
            val clientResponse = HttpResponse(
                HttpStatusCode.OK,
                mapOf("Content-Type" to listOf("application/octet-stream")),
                "placeholder-body-not-used",
            )
            every { mockHandleClientRequest.invoke(any()) } returns clientResponse

            val mockServeEvent = mockk<ServeEvent>(relaxed = true)
            val mockResponseDef = mockk<ResponseDefinition>(relaxed = true)
            every { mockResponseDef.bodyFileName } returns "large-response.bin"
            every { mockResponseDef.chunkedDribbleDelay } returns null
            every { mockServeEvent.responseDefinition } returns mockResponseDef
            every { mockWireMockServer.allServeEvents } returns listOf(mockServeEvent)

            // Simulate S3 streamer writing content to the output
            val s3Content = "streamed-from-s3-content"
            every { mockS3ResponseStreamer.streamToOutput(any(), any()) } answers {
                val outputStream = secondArg<OutputStream>()
                outputStream.write(s3Content.toByteArray())
                true
            }
            ...
            // Then — S3 streamer should be called with correct key
            verify(exactly = 1) { mockS3ResponseStreamer.streamToOutput("__files/large-response.bin", any()) }

            // Verify streaming protocol format: metadata + delimiter + S3 content
            val parsed = parseStreamingResponse(output.toByteArray())
            assertEquals(200, parsed.statusCode)
        ...
        @Test
        fun `Given S3 streaming failure When handling Then should abort stream and log error`() {
            // Given — S3 retrieval fails
            val input = createApiGatewayRequest("/mocknest/api/download", "GET")
            val output = ByteArrayOutputStream()
            val clientResponse = HttpResponse(
                HttpStatusCode.OK,
            ...
            val mockServeEvent = mockk<ServeEvent>(relaxed = true)
            val mockResponseDef = mockk<ResponseDefinition>(relaxed = true)
            every { mockResponseDef.bodyFileName } returns "missing-file.bin"
            every { mockResponseDef.chunkedDribbleDelay } returns null
            every { mockServeEvent.responseDefinition } returns mockResponseDef
            every { mockWireMockServer.allServeEvents } returns listOf(mockServeEvent)

            // S3 streamer returns false indicating failure
            every { mockS3ResponseStreamer.streamToOutput(any(), any()) } returns false

            // When
            handler.handleRequest(input, output, mockContext)

            // Then — S3 streamer should be called, stream aborted (no body content)
            verify(exactly = 1) { mockS3ResponseStreamer.streamToOutput("__files/missing-file.bin", any()) }

            // Metadata and delimiter should still be written
            val bytes = output.toByteArray()
            val delimiterIndex = findNullDelimiter(bytes)
            assertTrue(delimiterIndex >= 0, "Should contain metadata and delimiter even on S3 failure")
        }

        @Test
        fun `Given client request with bodyFileName When handling Then should write metadata before S3 streaming`() {
            // Given — verify metadata+delimiter is written before S3 content
            val input = createApiGatewayRequest("/mocknest/api/file", "GET")
            val output = ByteArrayOutputStream()
            val clientResponse = HttpResponse(
                HttpStatusCode.OK,
                mapOf("Content-Type" to listOf("application/pdf"), "X-Custom" to listOf("value")),
                "not-used",
            )
            every { mockHandleClientRequest.invoke(any()) } returns clientResponse
            ...
            val mockServeEvent = mockk<ServeEvent>(relaxed = true)
            val mockResponseDef = mockk<ResponseDefinition>(relaxed = true)
            every { mockResponseDef.bodyFileName } returns "report.pdf"
            every { mockResponseDef.chunkedDribbleDelay } returns null
            every { mockServeEvent.responseDefinition } returns mockResponseDef
            every { mockWireMockServer.allServeEvents } returns listOf(mockServeEvent)

            val pdfContent = "PDF-binary-content-here"
            every { mockS3ResponseStreamer.streamToOutput(any(), any()) } answers {
                val outputStream = secondArg<OutputStream>()
            ...
            // Then — verify metadata contains correct status and headers
            val parsed = parseStreamingResponse(output.toByteArray())
            assertEquals(200, parsed.statusCode)
            assertEquals("application/pdf", parsed.headers["Content-Type"])
            assertEquals("value", parsed.headers["X-Custom"])
            assertEquals(pdfContent, parsed.body)
        }

        @Test
        fun `Given large response body from client request without bodyFileName When handling Then should stream complete body`() {
            // Given — simulate a large response (1MB) without S3 reference
            val input = createApiGatewayRequest("/mocknest/api/download", "GET")
            val output = ByteArrayOutputStream()
            val largeBody = "A".repeat(1_048_576) // 1MB body
            val clientResponse = HttpResponse(
                HttpStatusCode.OK,
                mapOf("Content-Type" to listOf("application/octet-stream")),
                largeBody,
            ...
            // Then — complete body should be written to output stream (not via S3 streamer)
            verify(exactly = 0) { mockS3ResponseStreamer.streamToOutput(any(), any()) }
            val parsed = parseStreamingResponse(output.toByteArray())
            assertEquals(200, parsed.statusCode)
            assertEquals(largeBody.length, parsed.body.length)
            assertEquals(largeBody, parsed.body)
        }

        @Test
        fun `Given admin request with bodyFileName When handling Then should NOT use S3 streaming`() {
            // Given — S3 streaming only applies to client (/mocknest/) requests
            val input = createApiGatewayRequest("/__admin/mappings", "GET")
            val output = ByteArrayOutputStream()
            val adminResponse = HttpResponse(
                HttpStatusCode.OK,
            ...
            val mockServeEvent = mockk<ServeEvent>(relaxed = true)
            val mockResponseDef = mockk<ResponseDefinition>(relaxed = true)
            every { mockResponseDef.bodyFileName } returns "some-file.json"
            every { mockServeEvent.responseDefinition } returns mockResponseDef
            every { mockWireMockServer.allServeEvents } returns listOf(mockServeEvent)
            ...
            // Then — S3 streamer should NOT be called for admin requests
            verify(exactly = 0) { mockS3ResponseStreamer.streamToOutput(any(), any()) }
            val parsed = parseStreamingResponse(output.toByteArray())
            assertEquals(200, parsed.statusCode)
            assertEquals("""{"mappings":[]}""", parsed.body)
