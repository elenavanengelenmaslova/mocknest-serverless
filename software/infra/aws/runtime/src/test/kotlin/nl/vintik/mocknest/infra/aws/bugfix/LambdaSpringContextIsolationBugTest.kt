...
/**
 * Bug Condition Exploration Test for Runtime Lambda Spring Context Isolation
 * 
 * **CRITICAL**: This test is EXPECTED TO FAIL on unfixed code (monolithic module structure).
 * Test failure confirms the bug exists.
 * 
 * **Bug Description**: Runtime Lambda function fails during initialization
...
class LambdaSpringContextIsolationBugTest {
    ...
    /**
     * Property 1: Bug Condition - Runtime Lambda Initialization Success with FunctionCatalog
     * 
     * For Runtime Lambda initialization event where the Lambda is deployed from its own dedicated
     ...
     * 
     * **EXPECTED OUTCOME ON FIXED CODE**: This test WILL PASS because Runtime Lambda has its
     * own isolated Spring context with properly registered FunctionCatalog bean.
     */
    @Test
    fun `Property 1 - Given Runtime Lambda initialization When using dedicated module Then FunctionCatalog bean should exist in Spring context`() {
        try {
            // GIVEN: Runtime Lambda initialization with dedicated module structure
            val applicationClass = RuntimeApplication::class.java
            
            println("\n=== Testing Runtime Lambda Initialization ===")
            ...
            context = app.run(
                ...
                "--spring.cloud.function.definition=runtimeRouter",
            ...
            val functionCatalog = try {
                ...
                throw AssertionError(
                    "Bug confirmed: FunctionCatalog bean missing for Runtime Lambda. " +
            ...
            val expectedFunctionName = "runtimeRouter"
            ...
            if (function == null) {
                ...
                throw AssertionError(
                    "Bug confirmed: Function '$expectedFunctionName' not registered in FunctionCatalog for Runtime Lambda. " +
            ...
            if (!hasFunctionBean) {
                ...
                throw AssertionError(
                    "Bug confirmed: Function bean '$expectedFunctionName' not found in Spring context for Runtime Lambda. " +
                    "This is the expected failure on unfixed code with monolithic module structure."
                )
            }
            
            println("✓ Function bean '$expectedFunctionName' registered in Spring context")
            println("=== Runtime Lambda initialization PASSED ===\n")
            ...
            println("ERROR: Unexpected exception during Runtime Lambda initialization")
            println("Exception type: ${e::class.java.name}")
            println("Exception message: ${e.message}")
            e.printStackTrace()
            
            fail(
                "Bug confirmed: Runtime Lambda initialization failed with unexpected exception. " +
