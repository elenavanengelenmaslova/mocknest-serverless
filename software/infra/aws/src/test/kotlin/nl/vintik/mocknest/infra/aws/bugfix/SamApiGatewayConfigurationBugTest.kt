...
class SamApiGatewayConfigurationBugTest {
    ...
    private val projectRoot = File(System.getProperty("user.dir")).let { dir ->
        // Navigate up from software/infra/aws to project root
        if (dir.name == "aws") dir.parentFile.parentFile.parentFile else dir
    }
    private val samTemplatePath = File(projectRoot, "deployment/aws/sam/template.yaml")
    private val buildGradlePath = File(projectRoot, "software/infra/aws/build.gradle.kts")
    ...
    fun `Given Shadow plugin minimize WHEN building JAR THEN Spring Cloud adapter SHALL be preserved`() {
        ...
        val generationTaskStart = buildGradleContent.indexOf("shadowJarGeneration")
        
        assertTrue(
            runtimeTaskStart > 0,
            "Build configuration MUST have shadowJarRuntime task. " +
            "COUNTEREXAMPLE: shadowJarRuntime task not found."
        )
        
        assertTrue(
            generationTaskStart > 0,
            "Build configuration MUST have shadowJarGeneration task. " +
            "COUNTEREXAMPLE: shadowJarGeneration task not found."
        )
