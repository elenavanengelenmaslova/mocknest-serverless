...
4. **Deployment Configuration** (`deployment/sam/` and `deployment/sar/`) - Update SAM templates and deployment scripts
...
- Update `deployment/sam/template.yaml` when adding new Lambda functions or API Gateway endpoints
- Modify `deployment/sam/samconfig.toml` for environment-specific configurations
- Use the build and deploy scripts in `deployment/sam/` for consistent deployments
