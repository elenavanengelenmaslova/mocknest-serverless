...
- [x] 1. Add `AuthMode` parameter and conditions to SAM template (Req 1, Req 4)
  ...
  - [x] 1.1 Validate template structure after parameter and condition addition
...
- [x] 2. Apply conditional auth configuration to all three Lambda event sources (Req 1, Req 2, Req 3)
  ...
  - [x] 2.1 Validate conditional auth on all three event sources
...
- [x] 3. Make API key resources conditional on `API_KEY` mode (Req 2, Req 3)
  ...
  - [x] 3.1 Validate conditional resource exclusion
    - Run `sam validate --lint` and `cfn-lint`
...
- [x] 4. Checkpoint — SAM template complete
...
- [x] 5. Add `auth-mode` input to `sar-test-pipeline.yml` and ensure run isolation (Req 5, Req 7)
  ...
  - [x] 5.1 Validate pipeline YAML syntax
...
- [x] 6. Update stack output retrieval and API key step to be auth-mode-aware (Req 5)
  ...
  - [x] 6.1 Validate conditional output retrieval logic
...
- [x] 7. Implement auth-mode-specific health check authentication (Req 5)
  ...
  - [x] 7.1 Validate health check branching logic
...
- [x] 8. Update per-run summary report to include auth mode and failure details (Req 6)
  ...
  - [x] 8.1 Validate summary report content
...
- [x] 9. Checkpoint — `sar-test-pipeline.yml` complete
  - Ensure the reusable workflow file is valid YAML and all new inputs/steps are consistent
  - Ask the user if any questions arise.

- [x] 10. Split `sar-publish.yml` into two independent validation runs and combined summary (Req 5, Req 6, Req 7)
  ...
  - [x] 10.1 Validate `sar-publish.yml` structure
...
- [x] 11. Verify `GitHubOIDCAdmin` IAM role has `execute-api:Invoke` permission (Req 3, Req 5)
  ...
  - [x] 11.1 Validate OIDC role template
...
- [x] 16. Update docs/SAR_PUBLISHING.md
